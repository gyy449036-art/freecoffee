...
export function EmailManager({ initialSettings }: { initialSettings: EmailSettings | null }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/admin/email', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ host: data.get('host'), port: data.get('port'), username: data.get('username'), password, fromAddress: data.get('fromAddress'), replyTo: data.get('replyTo'), secure, enabled }) })
      const result = await response.json().catch(() => ({})) as { error?: string }
      if (!response.ok) { showToast(result.error || 'Unable to save email settings.'); return }
      setSettings({ host: String(data.get('host')), port: Number(data.get('port')), username: String(data.get('username') || ''), secure, fromAddress: String(data.get('fromAddress')), replyTo: String(data.get('replyTo') || '') || null, enabled })
      setPassword('')
  ...
  return <div className="media-manager">
    <section className="settings-card"><div className="connected-heading"><h2>Email delivery</h2><p>Configure SMTP delivery for receipts and creator notifications.</p></div>
          <form className="profile-form" onSubmit={save}>
            ...
            <div className="check-row"><Checkbox id="email-secure" checked={secure} onCheckedChange={(checked) => setSecure(checked === true)} /><label htmlFor="email-secure">Use TLS</label></div>
            <div className="check-row"><Checkbox id="email-enabled" checked={enabled} onCheckedChange={(checked) => setEnabled(checked === true)} /><label htmlFor="email-enabled">Enable email delivery</label></div>
