...
export type Product = { id: string; name: string; description?: string | null; price: number; currency: string }
...
export function GalleryTab({ creator, isAdmin, currentUser }: Pick<TabProps, 'creator' | 'isAdmin' | 'currentUser'>) {
  ...
  async function saveAlbum(values: { title: string; description: string; status: 'published' | 'draft' }) {
    if (!editing) return
    setBusy(true)
    await updateContent('gallery', editing.id, values)
    setBusy(false)
  }
...
export function PostsTab({ creator, isAdmin, currentUser }: Pick<TabProps, 'creator' | 'isAdmin' | 'currentUser'>) {
  ...
  const [deleting, setDeleting] = useState<string | null>(null)
  const [busy, setBusy] = useState(false)
  async function savePost(values: { title: string; excerpt: string; status: 'published' | 'draft' }) {
    if (!editing) return
    setBusy(true)
    await updateContent('post', editing.id, values)
    setBusy(false)
  }
  async function confirmDelete() {
    if (!deleting) return
...
export function ShopTab({ creator, currentUser, isAdmin }: Pick<TabProps, 'creator' | 'currentUser' | 'isAdmin'>) {
  ...
  const [deleting, setDeleting] = useState<string | null>(null)
  const [busy, setBusy] = useState(false)
  async function saveProduct(values: { name: string; description: string; price: number }) {
    if (!editing) return
    setBusy(true)
  ...
  return <>
    ...
    {creator.showShop === false ? <Placeholder title="Shop" text="This creator has hidden their shop." /> : creator.products?.length ? <section className="grid items-start justify-items-start gap-4 sm:grid-cols-2 lg:grid-cols-3">{creator.products.map((product) => <article className="relative flex aspect-[300/379] w-full max-w-[300px] min-w-0 flex-col overflow-hidden rounded-xl border bg-card shadow-sm" key={product.id}><a href={`/c/${encodeURIComponent(creator.handle)}/shop/${encodeURIComponent(product.id)}`} className="block"><CardCover alt="Product cover"><ShoppingBag className="size-10 text-muted-foreground/60" /></CardCover></a>{isAdmin && <CardActions onEdit={() => setEditing(product)} onDelete={() => setDeleting(product.id)} />}<div className="min-h-0 flex-1 p-4"><div className="flex h-full flex-col justify-between gap-3"><div><h2 className="truncate text-lg font-semibold"><a href={`/c/${encodeURIComponent(creator.handle)}/shop/${encodeURIComponent(product.id)}`} className="hover:underline">{product.name}</a></h2><p className="mt-2 line-clamp-2 text-sm leading-5 text-muted-foreground">{product.description || 'Digital download'}</p></div><strong className="text-xl">${(product.price / 100).toFixed(2)}</strong></div></div><form className="grid gap-2 border-t p-4" action="/api/shop/create" method="post"><input type="hidden" name="handle" value={creator.handle} /><input type="hidden" name="productId" value={product.id} /><Input required type="email" name="email" defaultValue={currentUser?.email ?? ''} placeholder="Buyer email" aria-label={`Buyer email for ${product.name}`} className="h-10 min-w-0" />{creator.paymentProviders?.stripe || creator.paymentProviders?.paypal ? <div className="flex min-w-0 gap-2"><Select name="provider" defaultValue={creator.paymentProviders?.stripe ? 'stripe' : 'paypal'}><SelectTrigger aria-label="Payment provider" className="h-10 min-w-0 flex-1"><SelectValue /></SelectTrigger><SelectContent><SelectGroup>{creator.paymentProviders?.stripe && <SelectItem value="stripe">Stripe</SelectItem>}{creator.paymentProviders?.paypal && <SelectItem value="paypal">PayPal</SelectItem>}</SelectGroup></SelectContent></Select><Button type="submit" className="shrink-0">Buy <ShoppingBag className="size-4" data-icon="inline-end" /></Button></div> : <span className="text-xs text-muted-foreground">Purchases are temporarily unavailable.</span>}</form></article>)}</section> : <Placeholder title="Shop" text="No products yet. Check back soon for digital tools and resources." />}
...
function EditContentDialog({ type, item, open, onOpenChange, busy, onSubmit }: { type: 'gallery' | 'post' | 'product'; item: { title?: string; description?: string | null; excerpt?: string | null; name?: string; price?: number; status?: 'published' | 'draft' }; open: boolean; onOpenChange: (open: boolean) => void; busy: boolean; onSubmit: (values: { title: string; description: string; status: 'published' | 'draft' } | { title: string; excerpt: string; status: 'published' | 'draft' } | { name: string; description: string; price: number }) => Promise<void> }) {
  ...
  const [price, setPrice] = useState(type === 'product' ? String((item.price ?? 0) / 100) : '')
  function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    if (!first.trim()) { showToast(type === 'product' ? 'Enter a product name.' : 'Enter a title.'); return }
    if (type === 'product') {
      const amount = Number(price)
      if (!Number.isFinite(amount) || amount < 0) { showToast('Enter a valid price.'); return }
      void onSubmit({ name: first.trim(), description: second.trim(), price: Math.round(amount * 100) })
    } else if (type === 'post') void onSubmit({ title: first.trim(), excerpt: second.trim(), status })
    else void onSubmit({ title: first.trim(), description: second.trim(), status })
  }
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>Edit {type}</DialogTitle><DialogDescription>Update this {type} using the fields below.</DialogDescription></DialogHeader><form className="grid gap-4" onSubmit={submit}><label className="grid gap-2 text-sm font-medium">{type === 'product' ? 'Product name' : type === 'post' ? 'Post title' : 'Album title'}<Input value={first} onChange={(event) => setFirst(event.target.value)} maxLength={160} /></label><label className="grid gap-2 text-sm font-medium">{type === 'post' ? 'Excerpt' : 'Description'}<Textarea value={second} onChange={(event) => setSecond(event.target.value)} /></label>{(type === 'gallery' || type === 'post') && <label className="grid gap-2 text-sm font-medium">Status<Select value={status} onValueChange={(value) => setStatus(value as 'published' | 'draft')}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectGroup><SelectItem value="published">Published</SelectItem><SelectItem value="draft">Draft</SelectItem></SelectGroup></SelectContent></Select></label>}{type === 'product' && <label className="grid gap-2 text-sm font-medium">Price (USD)<Input value={price} onChange={(event) => setPrice(event.target.value)} type="number" min="0" step="0.01" /></label>}<Button type="submit" disabled={busy}>{busy ? 'Saving...' : 'Save changes'}</Button></form></DialogContent></Dialog>
}
...
async function updateProduct(id: string, values: { name: string; description: string; price: number }) {
  try {
...
function AddGalleryDialog() {
  ...
  return <div className="flex justify-end"><Dialog open={open} onOpenChange={openDialog}><DialogTrigger asChild><Button type="button">Add album</Button></DialogTrigger><DialogContent className="max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>Add album</DialogTitle><DialogDescription>Add up to 8 images to one gallery album.</DialogDescription></DialogHeader><form className="space-y-4" onSubmit={submit}><div className="rounded-lg border border-dashed p-3"><div className="mb-3 flex items-center justify-between text-sm text-muted-foreground"><span>{files.length} of 8 images</span>{configured === false && <span className="text-destructive">S3 is not configured</span>}</div><div className="grid grid-cols-4 gap-2">{files.map((file, index) => <div className="relative aspect-square overflow-hidden rounded-md border" key={`${file.name}-${file.lastModified}-${index}`}><img src={URL.createObjectURL(file)} alt="" className="size-full object-cover" /><button type="button" className="absolute right-1 top-1 grid size-6 place-items-center rounded-full bg-background/90 text-sm" onClick={() => removeFile(index)} aria-label={`Remove ${file.name}`}><X className="size-3" /></button></div>)}{files.length < 8 && <label className="grid aspect-square cursor-pointer place-items-center rounded-md border border-dashed text-sm font-semibold hover:bg-muted"><span>Add +</span><input className="sr-only" type="file" accept="image/*" multiple onChange={selectFiles} disabled={configured !== true || busy} /></label>}</div></div><label className="grid gap-2 text-sm font-medium">Title<Input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={160} placeholder="Album title" /></label><label className="grid gap-2 text-sm font-medium">Description<Textarea value={description} onChange={(event) => setDescription(event.target.value)} maxLength={1000} placeholder="Describe this album" /></label><div className="grid gap-2"><Button className="w-full" type="submit" name="status" value="published" disabled={busy || configured !== true || !files.length}>{busy ? 'Uploading...' : 'Post album'}</Button><Button className="w-full" variant="outline" type="submit" name="status" value="draft" disabled={busy || configured !== true || !files.length}>Save draft</Button></div></form></DialogContent></Dialog></div>
}
...
function AddPostDialog() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true)
    try {
      const response = await fetch('/api/admin/content', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ type: 'post', title, excerpt, body, publish: (event.nativeEvent as SubmitEvent).submitter instanceof HTMLButtonElement && (event.nativeEvent as SubmitEvent).submitter.value === 'published' }) })
      const result = await response.json().catch(() => ({})) as { error?: string }
  ...
  return <div className="flex justify-end"><Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button type="button">Add post</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Add post</DialogTitle><DialogDescription>Save a new post as a draft.</DialogDescription></DialogHeader><form className="space-y-4" onSubmit={submit}><label className="grid gap-2 text-sm font-medium">Title<Input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={160} placeholder="Post title" /></label><label className="grid gap-2 text-sm font-medium">Excerpt<Textarea value={excerpt} onChange={(event) => setExcerpt(event.target.value)} rows={3} placeholder="A short summary" /></label><label className="grid gap-2 text-sm font-medium">Content<Textarea value={body} onChange={(event) => setBody(event.target.value)} rows={8} maxLength={20000} placeholder="Write your post..." /></label><div className="grid gap-2"><Button type="submit" name="status" value="published" disabled={busy}>{busy ? 'Adding...' : 'Publish post'}</Button><Button type="submit" name="status" value="draft" variant="outline" disabled={busy}>Save draft</Button></div></form></DialogContent></Dialog></div>
}
...
function AddProductDialog() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true)
    try {
      const response = await fetch('/api/admin/products', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name, description, price: Math.round(amount * 100), status: 'draft' }) })
      const result = await response.json().catch(() => ({})) as { error?: string }
  ...
  return <div className="flex justify-end"><Dialog open={open} onOpenChange={setOpen}><Button type="button" onClick={() => setOpen(true)}>Add product</Button><DialogContent><DialogHeader><DialogTitle>Add product</DialogTitle><DialogDescription>Save a digital product as a draft.</DialogDescription></DialogHeader><form className="space-y-4" onSubmit={submit}><label className="grid gap-2 text-sm font-medium">Product name<Input value={name} onChange={(event) => setName(event.target.value)} maxLength={160} placeholder="Product name" /></label><label className="grid gap-2 text-sm font-medium">Description<Textarea value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Describe the digital download" /></label><label className="grid gap-2 text-sm font-medium">Price (USD)<Input value={price} onChange={(event) => setPrice(event.target.value)} type="number" min="0" step="0.01" placeholder="10.00" /></label><Button type="submit" disabled={busy}>{busy ? 'Adding...' : 'Add product'}</Button></form></DialogContent></Dialog></div>
}
...
function SupportGoalCard({ creator, goal, isAdmin }: { creator: Creator; goal?: SupportGoal | null; isAdmin: boolean }) {
  ...
  return <section className="rounded-xl border bg-card p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h2 className="text-base font-semibold">{goal?.title ?? 'Support goal'}</h2>{!goal && <p className="mt-1 text-sm text-muted-foreground">Set a public goal and let supporters see what their contribution helps fund.</p>}</div><div className="flex gap-2">{goal && <Button type="button" variant="outline" size="icon" aria-label="Share support goal" onClick={() => void shareGoal()}><Share2 className="size-4" /></Button>}{isAdmin && <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button type="button" variant={goal ? 'outline' : 'default'}>{goal ? 'Edit goal' : 'Set a goal'}</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>{goal ? 'Edit support goal' : 'Set a support goal'}</DialogTitle><DialogDescription>Show supporters what you are working toward.</DialogDescription></DialogHeader><form className="space-y-4" onSubmit={saveGoal}><label className="grid gap-2 text-sm font-medium">Goal title<input className="h-10 rounded-md border bg-background px-3 font-normal outline-none focus-visible:border-ring" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Support open source development" maxLength={160} /></label><label className="grid gap-2 text-sm font-medium">Goal amount<input className="h-10 rounded-md border bg-background px-3 font-normal outline-none focus-visible:border-ring" type="number" min="1" step="1" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="100" /></label><label className="grid gap-2 text-sm font-medium">Description<textarea className="min-h-24 resize-y rounded-md border bg-background p-3 font-normal outline-none focus-visible:border-ring" value={description} onChange={(event) => setDescription(event.target.value)} maxLength={500} placeholder="Tell supporters what this goal will fund." /></label><Button type="submit" disabled={busy}>{busy ? 'Saving...' : 'Save goal'}</Button></form></DialogContent></Dialog>}</div></div>{goal && <><Progress className="mt-4 h-2" value={percent} aria-label={`${percent}% of support goal reached`} /><p className="mt-2 text-sm"><strong>{percent}%</strong> of ${(goal.amount / 100).toFixed(2)} goal</p>{goal.description && <p className="mt-3 text-sm leading-6 text-muted-foreground">{goal.description}</p>}</>}</section>
}
